    ...
    return (
        <>
            ...
            <Col className={styles.desc} span={7} offset={4}>
                <h2>{book.name}</h2>
                <Descriptions  >
                    <Descriptions.Item span={7} label="Author">{
                        book.author ?
                            <Link to={`/authors/${book.author._id}`} className={styles.link}>{`${book.author.firstname} ${book.author.lastname}`}</Link>
                            : null}
                    </Descriptions.Item>
                    <Descriptions.Item span={7} label="Category">{book.categories ?
                        book.categories.map(category => {
                            return <Link key={category._id} to={`/categories/${category._id}`}
                                className={styles.link}> {category.name}
                            </Link>
                        })
                        : null}
                    </Descriptions.Item>
                    <Descriptions.Item span={7} label="Average rate">{
                        book.rates ?
